...
export default function EditCampaignGroupDialog({
  open,
  onOpenChange,
  campaignGroup,
  onUpdateGroup,
  ...
  const [formData, setFormData] = useState<CampaignGroupFormData>({
    ...
    targetAcos: campaignGroup.targetAcos,
    presetGoal: campaignGroup.presetGoal,
    bidCeiling: campaignGroup.bidCeiling,
    bidFloor: campaignGroup.bidFloor,
  })

  const [errors, setErrors] = useState<Partial<Record<keyof CampaignGroupFormData, string>>>({})

  useEffect(() => {
    // Update form data when campaignGroup changes
    setFormData({
      name: campaignGroup.name,
      targetAcos: campaignGroup.targetAcos,
      presetGoal: campaignGroup.presetGoal,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle className="text-xl font-bold">Edit Campaign Group</DialogTitle>
        </DialogHeader>
